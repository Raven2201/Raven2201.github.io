...
CONFIG = {
    ...
    "weights": {
        "lvl_margin_pctl": 15.0, "lvl_mcap_pctl": 7.5, "lvl_liq_pctl": 7.5,
        "unwind_remaining": 22.0, "momentum": 8.0,
        "forced_amt_pctl": 10.0, "forced_ratio_pctl": 10.0,
        "vol_pctl": 10.0, "turnover_pctl": 10.0,
    },
    "etf_weight": 15.0,
    "signal2_manual": {"status": "watch", "note": "美股AI資本開支與台積電法說指引"},
    "signal3_manual": {"status": "watch", "note": "金管會融資成數/處置股規範動態"},
}
...
def compute(bulk, etf=None, hist=None):
    ...
    if partial:  # 部分數據時：滾動百分位序列停用；當期百分位改用5年分布骨架(若有)
        for key in P:
            P[key] = [None] * n
    KEYMAP = {"margin_total": "mt", "margin_val": "mval", "margin_mcap": "mmcap",
    ...
    parts = {
        ...
        "被動賣壓·融資淨減金額百分位": Wt["forced_amt_pctl"] * lp("bandae_amt") / 100,
        "被動賣壓·融資淨減率百分位": Wt["forced_ratio_pctl"] * lp("bandae_ratio") / 100,
    ...
    out = {
        ...
        "signals": {
            "s1": {"status": s1, "label": "技術性賣壓衰竭",
                   "detail": ("融資淨減5日均百分位 " + (str(last_pctl('bandae_amt')) if (not partial or sketch is not None) else "待完整歷史")
        ...
        "etf": etf or {"enabled": False, "note": "台股版口徑僅上市信用交易，不含上櫃/借券賣出/股票質押"},
    }
    return out
